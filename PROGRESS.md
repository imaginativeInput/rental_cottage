...
- **Deferred v-calendar off the critical path (headline).** It was imported + globally registered eagerly in `main.js` (`setupCalendar`/`Calendar`/`DatePicker` + `v-calendar/style.css`), dragging v-calendar + `@popperjs` + `date-fns` into the main chunk — yet the picker only ever renders behind a toggle. Now both consumers (`Reservation.vue` hero, `ReservationSection.vue`) define `VDatePicker` via `defineAsyncComponent` that dynamically imports v-calendar **and its CSS** on first open; `main.js` no longer touches v-calendar. `setupCalendar` is not needed — `DatePicker` self-provisions defaults. Naming the async const `VDatePicker` keeps the by-name test stub matching (a raw `import { DatePicker }` would break 27 tests). Browser-verified: the calendar still opens, renders (two-month PL layout, today dot, uppercase/arrow overrides), and selects a range. **Critical JS 130.7 → 68.3 KB gzip (entry 12.9 + vendor 55.9; v-calendar's 47 KB gzip now a lazy chunk). Critical CSS 9.42 → 4.81 KB gzip** (v-calendar's 28 KB stylesheet moved to a lazy chunk).
- **Removed dead `axios`.** `App.vue` imported axios only for an `onMounted` `axios.get('/api/data')` whose result was never rendered (`message` unused; no such endpoint — it SPA-fell-back to `index.html`). Deleted the import + call + unused `RouterLink`/`useI18n`; dropped `axios` from `package.json`. Kills a wasted request every load and ~13 KB gzip of dead weight.
- **vue-i18n production flags.** `vite.config.js` `define`s `__VUE_I18N_FULL_INSTALL__`/`__VUE_I18N_LEGACY_API__`/`__INTLIFY_PROD_DEVTOOLS__` = `'false'` (app is composition-API only, no `$t`/`<i18n-t>`). **Deliberately kept the message compiler** — messages are plain strings the runtime must compile; dropping it (or aliasing to the runtime-only build) made vue-i18n throw `unhandled node type: 0` at render in a real browser (the audit's test-only check missed this because tests stub i18n).
- **Vendor chunk for caching.** `build.rollupOptions.output.manualChunks` carves `vue`/`vue-router`/`pinia`/`vue-i18n` (~56 KB gzip) into a long-cached `vendor-*.js` so app-code edits don't bust the framework cache on repeat visits.
- **Pruned `@popperjs/core` + `date-fns` from direct deps** (only transitive via v-calendar; nothing in `src` imports them). `npm install` keeps them resolved under v-calendar. Dependency-hygiene; no bundle change.
- **`/galeria` grid serves responsive variants.** The grid `<img>` loaded full-res ~1920 px originals (~1.37 MB for 13 tiles) into ≤475 px cells. Generated `-480/-960/-1440` AVIF for the 7 grid names that lacked them (extended `generate-responsive-images.mjs`) and added `srcset` + `sizes="(min-width:1024px) 50vw, 100vw"` to the grid `<img>` (keeps `:src` as a graceful fallback). Browser-verified: all 13 tiles now fetch `-960` variants (e.g. `NZF_4358` 187 → 36 KB), **grid load ~1.37 MB → ~0.45 MB**. The lightbox still shows the bundled light image then swaps to `/gallery-hd/<name>.avif` (3 GalleryView tests green).
- **Gallery home slider:** dropped `fetchpriority="high"` from the first slide — it sits well below the hero and was competing with the LCP hero `<img>` for high priority.
- **`dns-prefetch`** to the reservation API origin in `index.html` (form POSTs there on submit).
- **Dead-code/file removal:** `src/assets/base.css` (unimported Vue scaffold), `HeaderTest.vue`, `ReservationForm.vue`, `stores/counter.js` (all zero refs), plus `Reservation.vue`'s `changeNumberOfGuests` (threw `ReferenceError` on an undefined `peopleCount`), `isCalendarOpen`, a stray `console.log`, an unused `sendReservationRequest` import, and a `[BUILD_DEBUG]` `console.warn` in `GalleryView`.
